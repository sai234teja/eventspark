...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      vec3 safe_tonemap(vec3 v) {
        // Safe alternative to tanh that avoids exp() overflow NaN
        return 1.0 - exp(-v);
      }
      ...
      void main() {
        ...
        for(int i = 0; i < ITERATIONS; i++) {
          ...
          float frequency = 1.0;
          float amplitude = 1.0;
          // Pre-calculate rotation for 0.4 radians
          const float wSin = 0.3894183423;
          const float wCos = 0.9210609940;
          
          for(int j = 0; j < WAVE_ITERATIONS; j++) {
            // Inline rotation: deformed.xz *= rot(0.4) using pre-computed
            float wx = deformed.x * wCos - deformed.z * wSin;
            float wz = deformed.x * wSin + deformed.z * wCos;
            deformed.x = wx;
            deformed.z = wz;
        ...
        color = safe_tonemap(color * uGlowAmount / widthNormalization);
